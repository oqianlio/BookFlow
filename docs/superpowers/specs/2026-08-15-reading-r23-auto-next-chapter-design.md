...
### 3c. 下一章后台预加载（无缝衔接，消除加载等待）

自动进入下一章时若走网络加载会经历「加载中…」状态，体验割裂。改进：

- **抓取逻辑抽取**：`loadChapter` 拆出 `fetchChapterData(c)`（持久缓存优先 → 网络抓取 → 解析内容/图片/nextContentUrl，文字章节同时写持久缓存）。
- **内存章节缓存** `chapterCacheRef`（上限 12 条，淘汰最旧）：已读章节与预取结果均写入。
- **后台预取**：当前章节加载成功后，立即按「nextContentUrl → 目录下一章」取下一章并调用 `prefetchChapter`（幂等，失败静默，结果写入内存缓存 + 持久缓存）。
- **无缝切换**：`loadChapter` 先查内存缓存，命中则直接渲染（无 loading）；`goChapter(±1)` 内存命中时同步应用内容再更新章节状态（同一批次渲染，无中间帧）。缓存未命中（如快速连翻）仍走正常加载路径。
- 漫画（图片章节）同样预取 `images` 数组，下一话无缝显示。
