...
### 3.52 搜索合并：本地+在线一次搜索，结果分区显示（2026-08-17）
- 场景：用户反馈"搜索书籍就有两个"——书架全文搜索（本地书）+ 发现页搜索
  （在线书源）分散在两个 Tab。合并到 DiscoverPage 一次搜索同时覆盖本地+在线。
- 实现：run 函数 `Promise.allSettled` 并行搜索本地（invoke search_books）+
  在线（searchBookSources），结果显示分区（"本地已导入"section + "在线书源"
  section）。本地搜索结果点击跳转到书中位置（onJumpLocal），在线结果点击
  进入详情页（onOpenBook）。
- 认知：**功能重复时先问"合并后体验是否更好"**——两个独立搜索入口合为一个
  结果更清晰；但合并时需处理两种结果的不同交互（本地跳转 vs 在线详情）。
  JSX 里冗余区块（搜索结果+空态都有频道）容易造成重复渲染，需仔细检查
  条件分支互斥。
...
### 3.38 详情页不要目录列表，一行最新章节即可（2026-08-16）
- 场景：用户先报"目录没必要列举这么多"，折叠后又说"不用展示
  目录，一行最新不就行了"。原版 legado 详情页有目录，但用户
  的阅读习惯是：详情页只留一行「最新章节」，章节选择进阅读页
  用目录面板（T 快捷键）。
- 认知：**UI 取舍以用户为准，原版只是起点**——上一轮刚按原版
  加了折叠目录，用户仍嫌多。删除目录列表后：`toc` 数据仍要
  内部抓取（缓存全书、开始阅读需要），只是不渲染；「最新章节」
...
### 3.40 批量健康检查会触发服务商限流，产生误报（2026-08-16）
- 场景：fullChainHealth 显示丁丁小说"目录提取为空"，实际是
  该服务商（api.xingliangglobal.com 与 api.jmlldsc.com 同源）
  返回 4011「请勿频繁操作」——批量并发 30 个同域名源请求触发
  限流，被误判为"目录 empty"。
- 认知：**同服务商多源并发 = 自找限流**。修复：健康检查按
  bookSourceUrl 域名分组，组内串行 + 200ms 间隔，组间 10 域名
...
### 3.45 健康检查必须复用真实链路，否则 JSON API 源全误报（2026-08-17）
- 场景：全链路健康检查显示南极"目录提取为空"，但手动走真实
  链路（bookUrl→ruleBookInfo→tocUrl→目录）完全正常。原因：
  健康检查的 tocStage 是简化实现（直接请求 bookUrl 页面提取
  目录），而 JSON API 源的目录在 ruleBookInfo.tocUrl 指向的
  另一个接口。
- 认知：**测试工具保真度决定结论可信度**（延续 2.4）——健康
  检查必须调用与真实阅读相同的代码路径。修复：抽取
  `fetchTocBySource`（给定已解析 BookSource 走完整目录链路），
  fetchToc 与健康检查共用。
- 下次：任何"检测/检查"类工具，先问"它走的链路和真实使用
  一致吗"；不一致就是误报源。

### 3.46 单括号 {$.x} 不是 legado 模板，规则错误直接修源（2026-08-17）
- 场景：阅友小说 bookUrl = `http://.../b/{$.wapBookId}.html`
  （单括号）→ 引擎不识别（legado 模板是 `{{...}}` 双括号）→
  bookUrl 变字面量 → 目录/正文全跳过（健康检查显示"无规则"）。
- 认知：**书源 JSON 里的规则错误（非引擎缺口）要直接修源**，
  不能盲目给引擎加兼容。修复：DB 里把单括号改双括号 →
  全链路达标（1102 章、503 字正文）。
- 下次：健康检查"无规则/跳环节"先看源 JSON 的规则串本身
  （括号、引号、转义），确认是源错还是引擎缺。

### 3.47 专有 App 环境（QQ 浏览器注入）的正文无法在桌面端获取（2026-08-17）
- 场景：南极正文接口 `/be-api/content/ads-read` 一直 -304；
  逆向 H5 发现正文页 `/qbread/adread/chapter` 检测 QQ 浏览器
  UA + 依赖 QIMEI/QUA2（QQ 浏览器内核注入的 JS 接口），CDP
  带 QQ UA 仍失败（headless 无注入环境）。
- 认知：**依赖专有 App 注入环境的源（如 QQ 阅读系）正文无法
  在桌面端获取**——与猫眼 alias-name 同类，识别后不再深挖；
  按标准保留（搜索/目录可用），正文记为质量缺陷。
- 下次：遇到"接口路径对但认证/环境失败"，先判断是否专有
  App 环境依赖，是则直接放弃深挖，记录结论。

### 3.48 备份/恢复：纯前端聚合现有 API，无需新 Rust 命令（2026-08-17）
- 场景：弹性目标——本地备份/恢复。盘点发现所有数据（书源、
  书架、进度、设置）都有现成 API，导出=前端聚合（list* +
  get*），导入=按 url 匹配更新（getSourceByUrl + update/add），
  零 Rust 改动。
- 认知：**先盘点现有 API 再决定是否加后端**——备份导出常是
  纯聚合逻辑；导入用"按业务主键匹配更新/新增"避免重复；
  版本号校验（BACKUP_VERSION）防旧文件误导入；主题/阅读设置
  走 localStorage 键。
- 下次：新功能先看"现有 API 能否拼出来"，聚合逻辑放
