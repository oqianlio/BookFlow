...
import { listBookSources, type BookSource } from "./api";
import { parseBookSourceJson } from "./bookSourceEngine";
import { fetchTocBySource } from "./sourceToc";
import { extractSingle } from "./bookSourceEngine";
...
async function checkSource(source: BookSource, keyword = "我的"): Promise<SourceHealth> {
  ...
  try {
    ...
    const searchHtml = await fetch(searchUrl, { signal: AbortSignal.timeout(8000) }).then(r => r.text()).catch(() => "");
    ...
    if (src.ruleContent?.content) {
      ...
      if (chapterUrl) {
        const contentHtml = await fetch(chapterUrl, { signal: AbortSignal.timeout(8000) }).then(r => r.text()).catch(() => "");
